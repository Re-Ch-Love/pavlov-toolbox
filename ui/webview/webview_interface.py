...
class PavlovToolboxWebsiteInterface(QWidget):
    ...
    def __init__(
        ...
        """
        Args:
            homeUrl (str): 主页Url，即默认打开的Url
            parent (QWidget | None, optional): 父控件. Defaults to None.
            isRemoveToolButtons (bool, optional): 是否移除工具栏（包括地址标签、回到主页按钮、前进、后退按钮）. Defaults to False.
            disableATagsJump (bool, optional): 是否禁用A标签的跳转，一般与isRemoveToolButtons一起使用，用来防止用户点击超链接后无法回到主页. Defaults to False.
        """
        super().__init__(parent)
        self.setObjectName(self.__class__.__name__ + "_" + uuid4().hex)
        self.initialQUrl = QUrl(homeUrl)
