...
from common.mod.mod_data import MOD_BATCH_REQUEST_URL, modBatchRequest, ModData
...
class ModDependenciesProcessor(QObject):
    ...
    def __init__(
        ...
        url = QUrl(DEPENDENCIES_URL % modData.resourceId)
...
def getModDependencies(request: QRequestReady, rid: int) -> QPromise:
    ...
    def dependenciesToModData(rids: List[int] | None) -> QPromise:
        ...
        if not rids:
            ...
            return request.get(MOD_BATCH_REQUEST_URL % ridsParam).then(
                lambda content: [ModData(item) for item in json.loads(content)["data"]]
            )

    return request.get(DEPENDENCIES_URL % rid).then(getDependencies).then(dependenciesToModData)
...
if __name__ == "__main__":
    ...
    (
        ...
        .then(lambda modDataList: print([modData.resourceId for modData in modDataList]))
        .done()
    )
    app.exec()
